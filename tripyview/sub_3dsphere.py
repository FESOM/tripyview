...
def create_3d_ocean_mesh(mesh,potatoefac=0.5,variable='elevation'):
...
def create_3d_land_mesh(mesh, resol=1, potatoefac=1, do_topo=False, topo_path=[], topo_varname='topo', topo_dimname=['lon','lat']):
...
def create_3d_lonlat_grid(dlon=30,dlat=15,potatoefac=1.0,do_topo=False):
    ...
    for nlonline in range(0,len(grid_lon)):
        if do_topo:
            xs,ys,zs = grid_cart3d(dum_lat*0+grid_lon[nlonline], dum_lat, R_earth+(6000*100*potatoefac), is_deg=True)
        else:    
            xs,ys,zs = grid_cart3d(dum_lat*0+grid_lon[nlonline], dum_lat, R_earth*1.005                , is_deg=True)
        ...
        aux_points = np.moveaxis(aux_points,0,1)
        aux_points = aux_points.reshape(3,2*aux_points.shape[1]).transpose()
        #aux_points = np.row_stack((aux_points,[np.nan,np.nan,np.nan]))

        points_lonlat_grid.append(aux_points)
        #if nlonline == 0 : points_lonlat_grid = aux_points
        #else :             points_lonlat_grid = np.row_stack((points_lonlat_grid,aux_points))
        del aux_points, points 
    ...
    for nlatline in range(0,len(grid_lat)):
        if do_topo:
            xs,ys,zs = grid_cart3d(dum_lon, dum_lon*0+grid_lat[nlatline], R_earth+(6000*100*potatoefac), is_deg=True)
        else:    
            xs,ys,zs = grid_cart3d(dum_lon, dum_lon*0+grid_lat[nlatline], R_earth*1.005, is_deg=True)
        ...
        points_lonlat_grid.append(aux_points)
        #points_lonlat_grid = np.row_stack((points_lonlat_grid,aux_points))
        del aux_points, points 
