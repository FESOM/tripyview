...
## `calc_zmoc(..., do_onelem=True)` — 2026-09-11

### 41. Seven independent bugs, the element path had apparently never run to completion — ✅ FIXED

Follow-up on the "still open" item from #38: `calc_zmoc(..., do_onelem=True)` crashed with `IndexError: index 1763 is out of bounds for axis 0 with size 48`. Fixing it uncovered a chain of six further latent bugs in the same code path -- each one only surfaced once the previous was fixed, consistent with this path never having been exercised successfully:

1. `data[vname][e_i, :]` / `data[vname][:, e_i, :]` indexed by axis **position**, assuming `('nod2', 'nz')` / `('time', 'nod2', 'nz')`. `load_data_fesom2` actually returns e.g. `('nz', 'nod2')` (horizontal last), so `e_i`'s node indices (up to n2dn) were applied to the depth axis (size nlev) -> the reported `IndexError`. Fixed with `.isel(nod2=e_i)`, which selects by dimension **name** and is safe regardless of axis order.
2. `data.where(mat_nzielem.data < mat_elemiz.data)` compared the raw `.data` arrays, i.e. by axis position again, and only "worked" by coincidence with the specific order the old (crashing) code happened to produce. Fixed by comparing the xarray objects directly, so alignment is by dimension name.
3. The bounds-coordinate cleanup `for vdrop in ['lon','lat','nodi','nodiz','w_A']: ...` was missing `ispbnd`, leaving a stray `nod2` dimension in the dataset that broke a `Dataset.transpose(...)` a few lines down (every dimension present must be named). Fixed by dropping any coordinate still carrying the `nod2` dimension, rather than a hardcoded name list.
4. The shared `moc_over_lat()` helper hardcoded `.isel(nod2=...)` / `dim='nod2'`, but the element path's horizontal dimension is `elem`. Fixed with `dhoriz = 'elem' if do_onelem else 'nod2'`.
5. The vertex path reduces to a plain `DataArray` (`data = data[vname]*data['w_A']*1e-6`) before `moc_over_lat()`; the element path never did the equivalent and stayed a one-variable `Dataset`, which cannot be assigned into a `DataArray` slot. Fixed by adding `data = data['w']` at the same point.
6. The subtlest one: `data['elemiz'].expand_dims({'nz': data['nzi']})` attaches `data['nzi']`'s *values* (level indices `0..nlev-1`) as the new dimension's coordinate **labels**. `data`'s own pre-existing `'nz'` coordinate is depth in metres (`0, 5, 10, 20, 30, 40, 50, ...`). The subsequent `data.where(mat_nzielem<mat_elemiz)` then auto-aligns by label and silently **intersects** the two mismatched label sets -- `{0,5,10,20,30,40,50,...}` (values (m)</code> that happen to also be <=nlev-1: 0,5,10,20,30,40, ...) intersected with `{0,1,...,47}` gave exactly 6 matches, silently collapsing the 48-level array to 6 without any error. Verified this exact mechanism against the raw file (`nz`=48 on disk) and by tracing the intermediate shapes step by step. Fixed by stripping the spurious `'nz'` coordinate (`reset_index('nz', drop=True)`) before the comparison, so it is purely positional as intended.
7. Unrelated to the above (pre-existing on `workbench`, affects both the element and vertex path whenever `do_parallel=True`): the joblib branch did `xr.concat(results, dim='nlat')` then `.transpose(..., 'lat')` -- two different names for the same new dimension, raising `ValueError: Dimensions {'lat'} do not exist`. Fixed by concatenating along `'lat'` directly, matching the existing (correct) `.transpose(...)` target name.

**Verified** (core2_pool, `dvd_core2_tke+idemix_GM1000_01`): `do_onelem=True` now runs to completion without a time dimension, with a time dimension, and via `do_parallel=True` (joblib), matching its own serial result exactly (`np.allclose(..., equal_nan=True)` -> True) in both cases; also confirmed `do_parallel=True` was equally broken for the vertex path (`do_onelem=False`) by the same bug #7 and is now fixed there too. Element-path AMOC max 15.44 Sv vs. vertex-path 15.51 Sv -- same order of magnitude; the two use different basin masks (element-centroid vs. vertex-based), so their latitude axis and exact value are not expected to match, only to agree physically.
